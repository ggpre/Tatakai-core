...
const TrendingPage = () => {
  ...
  useEffect(() => {
    const fetchTrending = async () => {
      try {
        setLoading(true);
        const response = await AnimeAPI.getAnimeByCategory('trending', page);
        if (page === 1) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
          <div className="flex items-center space-x-3 mb-4">
            ...
            <h1 className="text-3xl font-bold text-foreground">Trending Anime</h1>
          </div>
          <p className="text-muted-foreground">
            Discover what&apos;s hot and trending in the anime world right now.
          </p>
        </motion.div>
